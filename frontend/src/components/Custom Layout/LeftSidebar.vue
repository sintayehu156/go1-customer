  ...
  <div :class="['sidebar', { 'collapsed': isCollapsed }] " >
    ...
    <Button :variant="'ghost'" :class="{ 'active': isActive }" class="h-13" @click="isActive = !isActive">
        <template v-if="isCollapsed">
          <div v-if="logo">
                <Avatar
                  class="flex-shrink-0 rounded ml-[-2px]"
                  :shape="'square'"
                  image="/files/G0-1-Customer portal.png"
                  label="EY"
                  size="xl"                 
                />       
          </div>   
    ...
    <div v-if="dynamicsApp.data && dynamicsApp.data.length">
      <div v-for="(row, index) in dynamicsApp.data" :key="index">
        <Button 
          ...
          <!-- Collapsed View Icon -->
          <template v-if="isCollapsed">
            <FeatherIcon class="w-4 text-black flex justify-items-start ml-1" stroke="black" stroke-width="1" :name="row.icon" />
          </template>
    ...
    <!-- Toggle Sidebar Button -->
     <div class="toggle-button bg-gray-50 ">
      <Button class="mb-2 w-full justify-start" @click="$emit('toggle',this.isCollapsed)" :variant="'ghost'" size="sm">
            ...
            stroke="#000000" stroke-width="0.0030000000000000003" transform="matrix(1, 0, 0, 1, 0, 0)rotate(180)">
...
setup() {
  ...
  // fetch('/api/method/go1_customer.go1_customer.api.api.get_application_logo', {
  //   method: 'GET',
  //   headers: {
  //     'Expect': ''
  //   }
  ...
  // .then(data => {
  //   if (data.message && data.message.application_logo) {
  //     logoUrl.value = `/files${data.message.application_logo}`; 
...
const dynamicsAppLogo = createResource({
  url: 'go1_customer.go1_customer.api.api.get_navbar_routes_logo',
  cache: ['application_logo'],
  auto: true
});
dynamicsAppLogo.fetch()
console.log('3333',dynamicsAppLogo.fetch())
const logo=dynamicsAppLogo;
const logos=logo.data;
// console.log('app logo',dynamicsAppLogo)
// console.log('ssss',logos)
// console.log('dynamicddds:',  dynamicsApp);
// console.log('d222222ynamics data:', dynamicsApp.data);
//   console.log("usersdaata0----",users.fetch())
//   console.log("logo",dynamicsAppLogo.fetch())
//   console.log('ffffff',dynamicsAppLogo.data)
